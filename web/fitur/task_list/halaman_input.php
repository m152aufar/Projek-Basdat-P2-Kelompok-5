...
if (isset($_POST['simpan'])) {
    ...
    if (empty($error)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            // Get the inserted/updated task's ID
            $task_id = ($id != "") ? $id : mysqli_insert_id($koneksi);

            // Update or insert data into status table
            $sqlStatus = "INSERT INTO status (status_id, judul_status) VALUES (?, ?) ON DUPLICATE KEY UPDATE judul_status = ?";
            $stmtStatus = mysqli_prepare($koneksi, $sqlStatus);
            mysqli_stmt_bind_param($stmtStatus, "iss", $task_id, $judul_status, $judul_status);
            mysqli_stmt_execute($stmtStatus);

            // Update or insert data into mata_kuliah table
            $sqlMataKuliah = "INSERT INTO mata_kuliah (mata_kuliah_id, nama_mata_kuliah) VALUES (?, ?) ON DUPLICATE KEY UPDATE nama_mata_kuliah = ?";
            $stmtMataKuliah = mysqli_prepare($koneksi, $sqlMataKuliah);
            mysqli_stmt_bind_param($stmtMataKuliah, "iss", $task_id, $nama_mata_kuliah, $nama_mata_kuliah);
            mysqli_stmt_execute($stmtMataKuliah);
        ...
        mysqli_stmt_close($stmtMataKuliah);
    }
}
?>
...
<h1>New Task</h1>
<div class="mb-3 row">
    <a href="halaman.php">&lt;&lt; Kembali ke Task List</a>
</div>
<?php if ($error) { ?>
